# TODO: Translation updated at 2026-08-03 15:52

# game/Submods/MAICA_ChatSubmod/main.rpy:168

translate spanish maica_talking_ask_mspire_continue_e4353add:
    # m 1eub "Hmm...{w=0.3}shall we go further on this topic?{nw}"
    m 1eub "Hmm...{w=0.3} ¿quieres que sigamos hablando sobre este tema?{nw}"

# TODO: Translation updated at 2024-07-09 18:46

translate spanish strings:

    old "Go on, [player]"
    new "Continúa, [player]"
    old "A connection issue happend, maybe try again in a few minutes?"
    new "Parece que hubo un problema de conexión, inténtalo nuevamente en unos minutos"

# TODO: Translation updated at 2024-08-04 13:15

translate spanish strings:

    old "Something may went wrong..."
    new "Algo salió mal..."

# TODO: Translation updated at 2025-02-17 12:47

translate spanish strings:

    old "I'm done"
    new "Ya terminé"

# TODO: Translation updated at 2026-01-30 23:25

translate spanish strings:

    old "Failed replying mail, check submod_log.log for details\nError code: [ai.status] | [ai.MaicaAiStatus.get_description(ai.status)]"
    new "Error al responder la carta, revisa submod_log.log para más detalles\nCódigo de error: [ai.status] | [ai.MaicaAiStatus.get_description(ai.status)]"
    old "Failed replying mail. Not retrying because failure count limit reached"
    new "Error al responder la carta. Se alcanzó el límite máximo de intentos"

translate spanish strings:

    old "Hmm...shall we go further on this topic?{fast}"
    new "Hmm... ¿continuamos con este tema?{fast}"

    old "MAICA: Connection failed: "
    new "MAICA: Conexión fallida: "
    old "MAICA: Connection established"
    new "MAICA: Conexión establecida"
